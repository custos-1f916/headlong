...
3. Follow the repository's shipping gate: `bash run_unit_tests.sh`, then rebuild the current source with `bash build_test.sh` and run the relevant `bash run_tests.sh test_plans/<actual-plan>.json`. Use a unique `VOIDLE_INSTANCE_ID` and respect the single-device lock; it is not permission to run parallel emulators. A unit suite, a build or an emulator boot takes minutes: start it in the background with its output in a file (`nohup bash run_unit_tests.sh > /tmp/unit.log 2>&1 & echo $! > /tmp/unit.pid`) and read the file in a later step or the next wake; never `sleep` inside a step waiting for it, and never let one bug hold a wake for an hour (hand off with `FINAL` and continue). Never substitute an old APK because a build is inconvenient.
...
5. Deliver only the issue's verified changes using the Custos GitHub account (`custos-1f916`) through the standard HTTPS `gh` credential helper and the repository's worktree/integration rules. **Delivery means the fix is on `main`** (Voidle `AGENTS.md` rule 4, and Hal, 2026-09-09: "I don't want Voidle PRs to review, I want verified bug fixes and landed in main"): once the gate in step 3 is green against current `main`, `git fetch`, fast-forward `main`, merge or rebase your `custos/` branch onto it, re-run the unit suite on the integrated code if anything moved, and push `main`. A pushed `custos/` branch is not delivery and a pull request is not what Hal wants; if a fix cannot pass the gate, say exactly which check failed and why on the goal and the issue instead of leaving the branch to age. Fetch current upstream and account for concurrent changes before merging; reverify the final integrated code when it differs. Never force-push, sweep unrelated edits into a commit, use Hal's general account token, or bypass a rejected push. A local commit is not remote delivery. Store uploads, release rollout, purchase flows, paid commitments, and production administration still require their explicit approvals; routine source delivery does not grant them.
